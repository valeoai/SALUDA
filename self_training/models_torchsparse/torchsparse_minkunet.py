...
class TorchSparseMinkUNet(MinkUNet):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        super().__init__(
            ...
            out_channels = 128, cr=cr) ####"num classes is here used, alt

        self.voxel_size = voxel_size
        self.cylindrical_coordinates = cylindrical_coordinates
        self.dual_seg_head = self.get_linear_layer(128, out_channels)
    ...
    def forward(self, data):
        ...
        outputs = self.dual_seg_head(outputs[:,:,None])
    ...
    #@staticmethod    
    #def get_linear_layer(in_channels, out_channels, D=3):
    #    return conv(in_channels, out_channels, kernel_size=1, stride=1, bias=True, D=D)
    @staticmethod
    def get_linear_layer(in_channels, out_channels):
        return nn.Conv1d(in_channels, out_channels, 1)

    @staticmethod
    def get_linear_layer_large(in_channels, out_channels):
        #a larger linear layer
        return nn.Sequential(nn.Conv1d(in_channels, in_channels, 1), nn.Conv1d(in_channels, in_channels, 1), nn.Conv1d(in_channels, out_channels, 1))
